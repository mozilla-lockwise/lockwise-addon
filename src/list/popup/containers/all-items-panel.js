...
export default connect(
  (state, ownProps) => {
    ...
    const currProps = {...ownProps, totalItemCount};
    if (items.length || state.list.filter.userEntered) {
      return {...currProps, items, noResultsBanner: false, isFiltering};
    }
    // An autogenerated filter that produced no results; show everything, plus
    // a banner informing users.
    return {...currProps, items: state.cache.items, noResultsBanner: true};
  },
  (dispatch) => ({
