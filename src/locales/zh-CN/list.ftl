...
profile-menu-sign-in = 登录{ -sync-brand-short-name }服务
...
error-notification-duplicate-link = <a>要查看现有登录信息吗？</a>
...
intro-page-header-subtitle = 欢迎使用更好的登录信息管理工具。我们的桌面版附加组件中包含许多移动 App 中已推出的改进功能，让您更好地控制登录信息。
intro-page-main-article-1-title = 轻松管理账号
intro-page-main-article-1-copy = 管理账号密码绝非难事。我们提供全新界面，让事情变得更简单。
...
intro-page-footer-copy = { -fxlockwise-brand-name } 可让您在手机上使用保存至 { -firefox-brand-name } 的登录信息。若您在其他设备保存登录信息，可以登录或创建 { -fxaccount-brand-name }，即可同步信息到此设备上。<go>详细了解</go>
